...
final private class HookRepo:
  ...
  implicit private val creationOrdering: Ordering[Hook] = Ordering.by[Hook, Long](_.createdAt.getMillis)
  ...
  def bySid(sid: String) = hooks.values.find(_.sid.contains(sid))
  ...
  ): Vector[lila.pool.HookThieve.PoolHook] =
    hooks.values.withFilter(_.compatibleWithPool(clock, variant)).map(_.toPool).toVector
