...
final class Annotator(netDomain: lila.common.config.NetDomain) {
  ...
  def apply(p: Pgn, game: Game, analysis: Option[Analysis]): Pgn =
    annotateStatus(game.winnerSGPlayer, game.status, game.variant) {
  ...
  private def annotateStatus(winner: Option[SGPlayer], status: Status, variant: Variant)(p: Pgn) =
    ...
    if (drawOffers.isEmpty) pgn
      ...
      drawOffers.normalizedPlies.foldLeft(pgn) { case (pgn, ply) =>
        pgn.updatePly(
          ply,
          move => {
            val sgPlayer = !SGPlayer.fromPly(ply)
            move.copy(comments = s"$sgPlayer offers draw" :: move.comments)
          }
        )
      }
