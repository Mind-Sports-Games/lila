...
import strategygames.{ Board, Player => SGPlayer, Divider, Division, GameFamily, GameLogic, Piece, Replay, Role }
...
final private class PovToEntry(
            ...
            for {
              boards <-
                Replay
                  ...
                  .toOption
                  .flatMap(_.toNel)
              movetimes <- game.moveTimes(pov.sgPlayer).flatMap(_.toNel)
            } yield RichPov(
              pov = pov,
  ...
  private def makeMoves(from: RichPov): List[InsightMove] = {
    ...
    val prevInfos = from.analysis.?? { an =>
      Accuracy.prevSGPlayerInfos(from.pov, an) pipe { is =>
        from.pov.sgPlayer.fold(is, is.map(_.invert))
      }
    }
    val movetimes = from.movetimes.toList
    val roles     = from.pov.game.pgnMoves(from.pov.sgPlayer).map(pgnMoveToRole(from.pov.game.variant.gameFamily, _))
    val boards = {
      val pivot = if (from.pov.sgPlayer == from.pov.game.startSGPlayer) 0 else 1
    ...
    movetimes.zip(roles).zip(boards).zip(blurs).zip(timeCvs).zipWithIndex.map {
      case (((((movetime, role), board), blur), timeCv), i) =>
        val ply      = i * 2 + from.pov.sgPlayer.fold(1, 2)
        ...
        InsightMove(
          ...
          cpl = cpDiffs lift i,
          material = board.materialImbalance * from.pov.sgPlayer.fold(1, -1),
  ...
  private def queenTrade(from: RichPov) =
    QueenTrade {
      from.division.end.fold(from.boards.last.some)(from.boards.toList.lift) match {
        case Some(board) =>
          SGPlayer.all.forall { sgPlayer =>
            !board.hasPiece(Piece(GameLogic.Chess(), sgPlayer, Role.ChessRole(strategygames.chess.Queen)))
          }
        case _ =>
  ...
  private def convert(from: RichPov): Option[InsightEntry] = {
    ...
    for {
      ...
      myCastling = Castling.fromMoves(game pgnMoves pov.sgPlayer),
      opponentRating = opRating,
      opponentStrength = RelativeStrength(opRating - myRating),
      opponentCastling = Castling.fromMoves(game pgnMoves !pov.sgPlayer),
