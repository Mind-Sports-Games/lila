...
object PerfPicker {
  ...
  def perfType(speed: Speed, variant: strategygames.chess.variant.Variant, daysPerTurn: Option[Int]): Option[PerfType] =
    PerfType(key(speed, variant, daysPerTurn))

  def key(speed: Speed, variant: strategygames.chess.variant.Variant, daysPerTurn: Option[Int]): String =
    if (variant.standard) {
  ...
  def main(speed: Speed, variant: strategygames.chess.variant.Variant, daysPerTurn: Option[Int]): Option[Perfs => Perf] =
  ...
  def mainOrDefault(speed: Speed, variant: strategygames.chess.variant.Variant, daysPerTurn: Option[Int]): Perfs => Perf =
    main(speed, variant, daysPerTurn) orElse {
      (variant == strategygames.chess.variant.FromPosition) ?? main(speed, strategygames.chess.variant.Standard, daysPerTurn)
    } getOrElse default
