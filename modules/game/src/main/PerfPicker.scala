...
object PerfPicker {
  ...
  def key(speed: Speed, variant: Variant, daysPerTurn: Option[Int]): String =
    if variant == Variant.Chess(Standard) then
      if daysPerTurn.isDefined || speed == Speed.Correspondence then Speed.Correspondence.key
      else speed.key
    else variant.key
  ...
  def mainOrDefault(speed: Speed, variant: Variant, daysPerTurn: Option[Int]): Perfs => Perf =
    main(speed, variant, daysPerTurn) orElse {
      (variant == Variant.Chess(FromPosition)) so main(speed, Variant.Chess(Standard), daysPerTurn)
    } getOrElse default
