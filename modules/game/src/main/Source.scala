...
object Source {
  ...
  case object BotVsBotStream extends Source(id = 14)

  val all  =
    List(Lobby, Friend, Ai, Api, Tournament, Position, Import, Simul, Relay, Pool, Swiss, BotVsBotStream)
  val byId = all map { v =>
    (v.id, v)
  } toMap

  val searchable             =
    List(Lobby, Friend, Ai, Position, Import, Tournament, Simul, Pool, Swiss, BotVsBotStream)
