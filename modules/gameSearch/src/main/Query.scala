...
case class Query(
    ...
    fullTurnsCompleted: Range[Int] = Range.none,
  ...
  def nonEmpty =
    user1.nonEmpty ||
      ...
      winner.nonEmpty ||
      loser.nonEmpty ||
      winnerPlayerIndex.nonEmpty ||
      perf.nonEmpty ||
      source.nonEmpty ||
      status.nonEmpty ||
      fullTurnsCompleted.nonEmpty ||
...
object Query {
  ...
  val winnerPlayerIndexs = List(1 -> "Player 1", 2 -> "Player 2", 3 -> "None")
  ...
  val fullTurnsCompleted = options(
