...
class CommentParserTest extends munit.FunSuite {

  given variant: Variant = Variant.libStandard(GameLogic.Chess())
  val C                  = CommentParser
  ...
  test("parse shapes: at start") {
    val result = C("[%csl Gb4,Yd5,Rf6] Hello there")
    assertEquals(result.comment, "Hello there")
    assertEquals(result.clock, None)
    assertEquals(result.shapes.value.size, 3)
  }
  test("parse shapes: at end") {
    val result = C("Hello there [%csl Gb4,Yd5,Rf6]")
    assertEquals(result.comment, "Hello there")
    assertEquals(result.clock, None)
    assertEquals(result.shapes.value.size, 3)
  }
  test("parse shapes: multiple") {
    val result = C("Hello there [%csl Gb4,Yd5,Rf6][%cal Ge2e4,Ye2d4,Re2g4]")
    assertEquals(result.comment, "Hello there")
    assertEquals(result.clock, None)
    assertEquals(result.shapes.value.size, 6)
  }
  test("parse shapes: new lines") {
    val result = C("Hello there [%csl\nGb4,Yd5,Rf6]")
    assertEquals(result.clock, None)
    assertEquals(result.shapes.value.size, 3)
  }
  test("parse shapes: multiple, one new line") {
    assertEquals(C("Hello there [%csl\nGb4,Yd5,Rf6][%cal Ge2e4,Ye2d4,Re2g4]").shapes.value.size, 6)
    assertEquals(C("Hello there [%csl Gb4,Yd5,Rf6][%cal\nGe2e4,Ye2d4,Re2g4]").shapes.value.size, 6)
  }
  test("parse shapes: multiple mess") {
    assertEquals(C("Hello there [%csl \n\n Gb4,Yd5,Rf6][%cal\nGe2e4,Ye2d4,Re2g4]").shapes.value.size, 6)
  }

  test("parse all: multiple shapes + clock") {
    val result = C("Hello there [%clk 10:40:33][%csl \n\n Gb4,Yd5,Rf6][%cal\nGe2e4,Ye2d4,Re2g4]")
    assertEquals(result.shapes.value.size, 6)
    assertEquals(result.clock, Some(Centis(3843300)))
  }
}
