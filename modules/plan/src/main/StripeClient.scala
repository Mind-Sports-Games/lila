    ...
    val args = sessionArgs(data) ++ List(
      ...
      "line_items[][description]" -> {
        if (data.checkout.amount.value >= 25000)
          s"Lifetime Patron status on playstrategy.org. <3 Your support makes a huge difference!"
        else
          s"One month of Patron status on playstrategy.org. <3 Your support makes a huge difference!"
      }
    )
