...
class UciToPgnTest extends munit.FunSuite {
  ...
  test("convert UCI analysis to PGN - work :)") {
    ...
    val (_, errs) = UciToPgn(rep, uciAnalysis)
    assert(errs.isEmpty)
  }

  test("convert UCI analysis to PGN - even in KotH") {
    val pgn = List(
      "e4",
      "e5",
      "d4",
