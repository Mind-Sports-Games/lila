...
<a href="${routes.Auth.login}?referrer=${ctx.req.path}" class="signin button button-empty">${trans.signIn
  ...
  val dataMainColor             = attr("data-main-color")
  ...
  def apply(
    ...
    frag(
      ...
      htmlTag(ctx.lang)(
        ...
        st.body(
          cls := List(
            s"${ctx.currentBg} ${ctx.currentMainColour} ${current2dTheme} ${ctx.currentTheme3d.cssClass} ${ctx.currentPieceSet3d.toString} coords-${ctx.pref.coordsClass}" -> true,
          ...
          dataMainColor := ctx.currentMainColourCls,
