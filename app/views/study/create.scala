...
object create {
  ...
  def apply(
    ...
    views.html.site.message(
      ...
      icon = Some("4"),
      back = data.actualFen.map(f => routes.Editor.loadFenWithVariant(f.value, data.variantStr.getOrElse("standard")).url),
      ...
      div(cls := "study-create")(
        postForm(action := routes.Study.create)(
          ...
          input(tpe := "hidden", name := "fen", value := data.actualFen.map(_.value)),
