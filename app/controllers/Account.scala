...
final class Account(
  ...
  private def refreshSessionId(me: UserModel, result: Result)(implicit
      ctx: Context,
    ...
    AuthBody { implicit ctx => me =>
      auth.HasherRateLimit(me.username, ctx.req) { _ =>
        ...
        env.security.forms.setupTwoFactor(me) flatMap { form =>
          FormFuResult(form)(err => fuccess(html.account.twoFactor.setup(me, err))) { data =>
      ...
      env.security.reopen.confirm(token) flatMap {
        ...
        case Some(user) =>
          env.report.api.reopenReports(lila.report.Suspect(user)) >>
            auth.authenticateUser(user).andDo {
              val _ = lila.mon.user.auth.reopenConfirm("success").increment()
            }
      }
    }
