...
final class PlayApi(
  ...
  implicit private def autoReqLang(implicit req: RequestHeader): Lang = reqLang(using req)
  ...
  private object impl {
    ...
    def gameStream(me: UserModel, pov: Pov)(implicit lang: Lang) =
      env.game.gameRepo.withInitialFen(pov.game) map { wf =>
        apiC.sourceToNdJsonOption(env.bot.gameStateStream(wf, pov.playerIndex, me))
      }
