      ...
      Reasonable(page) {
        ...
        OptionFuResult(userOption) { user =>
          path match {
            ...
            case "shieldleaderboard" =>
              env.tournament.leaderboardApi.shieldLeaderboardByUser(user, page).map { entries =>
                Ok(html.userTournament.bits.shieldLeaderboard(user, entries))
              }
            case "best" =>
