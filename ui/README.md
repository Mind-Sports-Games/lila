...
NOTE - since we now use pnpm, it may be useful to know you can run
`rm -rf node_modules pnpm-lock.yaml && && pnpm i`
if you want to ensure pnpm tries to re-solve completely the deps tree (from the root dir of this project:
`rm -rf node_modules pnpm-lock.yaml && find ./ui -type d -name node_modules -prune -exec rm -rf '{}' + && pnpm i` to also clear the node_modules of )\
Previously we used to also include `pnpm store prune` but this caused errors for some people and running the clean command without that part did the job. Previous full clean command was:
`rm -rf node_modules pnpm-lock.yaml && find ./ui -type d -name node_modules -prune -exec rm -rf '{}' + && pnpm store prune && pnpm i`
