...
export function renderSvg(state: State, root: SVGElement): void {
  const d = state.drawable,
    curD = d.current,
    cur = curD && (curD.mouseSq || curD.prev) ? (curD as DrawShape) : undefined,
...
function syncShapes(
  ...
  while (el) {
    ...
    if (Object.prototype.hasOwnProperty.call(hashesInDom, elHash)) hashesInDom[elHash] = true;
    // or remove it
...
function pos2px(pos: cg.Pos, bounds: ClientRect, boardSize: cg.BoardSize): cg.NumberPair {
  return [
    ((2 * pos[0] - (pos[1] % 2 !== 0 ? 0.5 : 1.5)) * bounds.width) / boardSize[0],
    ((pos[1] - 0.5) * bounds.height) / boardSize[1],
  ];
}
