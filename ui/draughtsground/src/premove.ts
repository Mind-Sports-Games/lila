...
export default function premove(pieces: cg.Pieces, boardSize: cg.BoardSize, key: cg.Key, variant?: string): cg.Key[] {
  ...
  const frisianVariant = variant && (variant === 'frisian' || variant === 'frysk'),
    menOnlyForwardVariant = variant && (variant === 'portuguese' || variant === 'english'),
    nonFlyingKingsVariant = variant && variant === 'english',
  ...
  if (piece.role === 'man' || nonFlyingKingsVariant) {
    //
    ...
    if (piece.role === 'king' || !menOnlyForwardVariant || piece.playerIndex === 'p1') {
      for (let i = 0; i < (frisianVariant ? 3 : 2); i++) {
        let f = movesUp[field][i];
        if (f != -1) {
          const key = field2key(f);
          if ((piece.playerIndex === 'p1' && i < 2) || piece.role === 'king') dests.push(key);
    ...
    if (piece.role === 'king' || !menOnlyForwardVariant || piece.playerIndex === 'p2') {
      for (let i = 0; i < (frisianVariant ? 3 : 2); i++) {
        let f = movesDown[field][i];
        if (f != -1) {
          const key = field2key(f);
          if ((piece.playerIndex === 'p2' && i < 2) || piece.role === 'king') dests.push(key);
