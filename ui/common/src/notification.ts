...
function notify(msg: string | (() => string)) {
  const storage = playstrategy.storage.make('just-notified');
  if (document.hasFocus() || Date.now() - parseInt(storage.get()!, 10) < 1000) return;
  storage.set('' + Date.now());
  if ($.isFunction(msg)) msg = msg();
  const notification = new Notification('playstrategy.org', {
    icon: playstrategy.assetUrl('logo/playstrategy-favicon-256.png', { noVersion: true }),
    body: msg,
  });
